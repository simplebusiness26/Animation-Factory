...
def fetch_public_page(slug: str, label: str, suffix: str) -> tuple[str, str]:
    ...
    try:
        ...
        parser.feed(r.text)
        text = parser.text()
        # A real Kaggle competition page normally produces substantial visible text.
        if len(text) < 300:
            return label, ""
...
def data_fit(files: list[dict[str, str]], task: str) -> tuple[float, int, str]:
    ...
    gb = total / 1024**3
    if gb <= 5:
        return 90.0, total, f"dataset footprint about {gb:.1f} GB"
    if gb <= 20:
        return 75.0, total, f"dataset footprint about {gb:.1f} GB"
    if gb <= 50:
        return 50.0, total, f"large dataset footprint about {gb:.1f} GB"
...
def build(item: dict[str, Any]) -> Dossier:
    ...
    pages = fetch_pages(slug)
    files = fetch_files(slug)
    topics = fetch_topics(slug)
    text = " ".join(pages.values())

    task, compute, task_reason = infer_task(text, slug)
    tf = time_fit(days)
    cf = crowd_fit(teams)
    df, total, data_reason = data_fit(files, task)
    doc = 95.0 if len(pages) >= 2 else 75.0 if pages else 35.0

    flags: list[str] = []
    reasons = [task_reason, data_reason]

    dep = has_pattern(text.lower(), DEPENDENCY_PATTERNS)
    dep_penalty = 25.0 if dep else 0.0
    if dep:
        flags.append("hard prerequisite/dependency detected")
        reasons.append("public competition text requires participation or a submission in another track/competition")

    elig = has_pattern(text.lower(), ELIGIBILITY_PATTERNS)
    elig_penalty = 8.0 if elig else 0.0
    if elig:
        flags.append("age/residency/eligibility terms require review")
        reasons.append("eligibility language detected; do not enter until the official rules are checked")

    if not pages:
        flags.append("public Kaggle pages could not be read automatically")
        reasons.append("documentation confidence reduced")
    else:
        reasons.append(f"read public pages: {', '.join(sorted(pages))}")
    if topics:
        reasons.append(f"inspected metadata for {len(topics)} top discussion topics")
    ...
    return Dossier(
        ...
        compute_fit=compute,
        time_fit=tf,
        crowd_fit=round(cf, 1),
        data_fit=df,
        documentation_fit=doc,
        dependency_penalty=dep_penalty,
        eligibility_penalty=elig_penalty,
        final_score=final,
        verdict=verdict,
        public_pages_read=sorted(pages),
...
def detail_md(d: Dossier) -> str:
    ...
    lines = [
        ...
        f"**Stage 1 score:** {d.stage1_score:.1f}/100",
        f"**Prize:** {d.reward}",
        f"**Teams:** {d.teams if d.teams else 'unknown'}",
        f"**Days left:** {d.days_left:.0f}",
        f"**Task type:** {d.task_type}", "",
        "## Feasibility", "",
        f"- Compute fit: **{d.compute_fit:.0f}/100**",
        ...
        f"- Public pages read: **{', '.join(d.public_pages_read) if d.public_pages_read else 'none'}**",
        f"- Competition files visible: **{d.files_count}**",
        f"- Visible data size: **{gb:.2f} GB**" if d.total_data_bytes else "- Visible data size: **unknown / not exposed**",
        "",
    ]
    if d.flags:
        lines += ["## Flags", ""] + [f"- ⚠️ {x}" for x in d.flags] + [""]
    lines += ["## Reasons", ""] + [f"- {r}" for r in d.reasons] + [
        "",
        "## Entry gate", "",
...
def summary_md(ds: list[Dossier]) -> str:
    ...
    lines = [
        ...
        f"Generated: **{now}**", "",
        "Stage 2 reranks the top Stage 1 opportunities using public competition pages plus Kaggle metadata.", "",
    ...
    if ordered:
        lines += ["", "## Best current target", "", f"**{ordered[0].slug}** — {ordered[0].verdict}, **{ordered[0].final_score:.1f}/100**.", ""]
    return "\n".join(lines)
...
def main() -> int:
    ...
    items = (payload.get("opportunities") or [])[: max(1, min(args.top, 10))]
    OUT.mkdir(parents=True, exist_ok=True)

    ds = [build(item) for item in items]
    for d in ds:
        (OUT / f"{d.slug}.md").write_text(detail_md(d) + "\n", encoding="utf-8")
        (OUT / f"{d.slug}.json").write_text(json.dumps(asdict(d), indent=2) + "\n", encoding="utf-8")

    (OUT / "latest.md").write_text(summary_md(ds) + "\n", encoding="utf-8")
    (OUT / "latest.json").write_text(json.dumps({"generated_at": datetime.now(timezone.utc).isoformat(), "dossiers": [asdict(d) for d in ds]}, indent=2) + "\n", encoding="utf-8")

    ordered = sorted(ds, key=lambda d: d.final_score, reverse=True)
    print(f"Built {len(ds)} Stage 2 public dossiers.")
    if ordered:
