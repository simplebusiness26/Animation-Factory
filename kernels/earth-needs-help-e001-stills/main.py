...
This runner is deliberately fail-closed. It will not generate recurring
characters from prose alone. Every shot is conditioned on the locked visual
character pack from shows/earth-needs-help/assets/characters and repeats the
canonical identity contract in the text prompt.

The previous recovery runner used one bridge image plus SDXL img2img strengths
around 0.86-0.90. That gave the diffusion model enough freedom to redesign the
cast. This version removes that route and uses IP-Adapter Plus with a fresh
contact sheet built from the locked character references on every shot.
"""
...
CANONICAL_LOCK = (
    'The attached reference sheet is the character source of truth. '
    'Reproduce the SAME recurring character designs; do not reinterpret or redesign them. '
    'Captain Pip is the GREEN rounded alien captain with large expressive eyes and the exact captain styling shown in the reference. '
    ...
    'When the Earth child is visible, use the exact child design from the reference. '
    ...
    'Polished bright friendly 3D children\'s comedy animation, rounded forms, warm cinematic lighting, 16:9 landscape. '
)
...
NEGATIVE = (
    ...
    'changed costume, missing captain styling, duplicate character, extra limbs, missing limbs, deformed face, '
...
def load_manifest() -> dict:
    return json.loads(fetch_bytes(MANIFEST_PATH).decode('utf-8'))
...
def preflight(manifest: dict) -> tuple[Path, dict[str, str]]:
    ...
    if not bool((manifest.get('canon_policy') or {}).get('load_references_for_every_shot')):
        raise RuntimeError('CONTINUITY_BLOCK: per-shot reference loading is disabled')
    if bool((manifest.get('canon_policy') or {}).get('text_only_recurring_character_generation_allowed')):
    ...
    for filename in required:
        data = fetch_bytes(f'{directory}/{filename}')
        target = local_dir / filename
        target.write_bytes(data)
        refs[Path(filename).stem] = str(target)
...
def build_reference_sheet(refs: dict[str, str], visible: list[str]):
    ...
    full_cast = refs.get('full-cast')
    if full_cast:
        with Image.open(full_cast) as im:
            selected.append(im.convert('RGB').copy())
...
def main() -> int:
    ...
    result = {
        ...
        'episode': '001',
        'success': False,
        'reference_mode': 'locked individual refs + full-cast visual reference -> IP-Adapter Plus on every shot',
    ...
    for index, shot in enumerate(SHOTS):
        ...
        try:
            ...
            prompt = (
                CANONICAL_LOCK
                + ' Compose a NEW production scene using those exact recurring designs. '
                + shot['prompt']
                + ' Single clean production frame only. No text.'
            )
            image = pipe(
                prompt=prompt,
            ...
            result['shots'].append({
                ...
                'success': True,
                'file': output_name,
                'references_loaded': shot['characters'] + ['full-cast']
            })
