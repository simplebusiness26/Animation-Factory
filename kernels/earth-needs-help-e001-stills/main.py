...
SHOTS = [
 {'id':'001','prompt':'Colourful alien spaceship bridge during a gentle emergency alert. Captain Pip snaps upright at the controls; Bloop gasps; Zig reacts to his gadget; Momo stays calm. Earth visible through the front window. Friendly cinematic 3D animation.'},
...
def main()->int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError('GPU_BLOCK: CUDA required')
    pipe.to('cuda')
    if hasattr(pipe,'enable_vae_tiling'):
        pipe.enable_vae_tiling()
    result={'show':'Earth Needs Help','episode':'001','success':False,'reference_mode':'validated canon sheet -> explicit ViT-H -> SDXL IP-Adapter Plus','continuity_manifest':MANIFEST_PATH,'reference_files':[p for _,p,_ in REFERENCE_FILES],'master_reference_staged_as':str(master_path),'image_encoder_hidden_size':1280,'ip_adapter_scale':0.90,'gpu':torch.cuda.get_device_name(0),'shots':[]}
    for index,shot in enumerate(SHOTS):
        ...
        try:
            ...
            result['shots'].append({'id':shot['id'],'success':True,'file':name})
    ...
    result['success']=all(x.get('success') for x in result['shots']) and len(result['shots'])==len(SHOTS)
    write_report(result)
